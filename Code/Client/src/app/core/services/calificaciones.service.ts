...
export interface EstudianteNotas {
  estudianteId: string;  // Código del estudiante (ej: "EST001")
...
export interface GrupoDocente {
  grupoId: string;  // Código del paralelo (alfanumérico como "SIS-101-A")
  materiaNombre: string;
  grupoCodigo: string;
...
export class CalificacionesService {
   ...
   * USANDO: GET /api/calificaciones/estudiante/{codigo}
   */
  async obtenerMisNotas(): Promise<NotaMateria[]> {
    ...
    console.log('=== obtenerMisNotas ===');
    console.log('Buscando calificaciones para estudiante:', userCodigo);

    try {
      // Usar endpoint de calificaciones (donde se guardan las notas)
      const dtoCalificaciones = await firstValueFrom(
        this.api.get<any[]>(`/calificaciones/estudiante/${userCodigo}`)
      );

      console.log(`${dtoCalificaciones.length} calificaciones obtenidas del backend:`, dtoCalificaciones);
      ...
      dtoCalificaciones.forEach(calif => {
        const paraleloId = calif.evaluacion?.paraleloMateria?.codigo || 'unknown';
        console.log('Procesando calificacion para paralelo:', paraleloId, calif);
      ...
      calificacionesPorParalelo.forEach((califs, paraleloId) => {
        ...
        const paralelo = primeraCalif.evaluacion?.paraleloMateria;
        const materia = paralelo?.materia;

        console.log('Paralelo:', paralelo, 'Materia:', materia);

        if (!materia) {
          console.warn('Materia no encontrada para paralelo:', paraleloId);
          return;
        }
        ...
        const evaluaciones: EvaluacionNota[] = califs.map(calif => {
          ...
          // El backend usa 'valor' no 'nota'
          const nota = calif.valor ?? calif.nota;
          ...
          return {
            id: parseInt(calif.codigo) || Date.now(),
            nombre: evaluacion?.nombre || evaluacion?.descripcion || 'Evaluación',
        ...
        notasMaterias.push({
          materiaId: parseInt(materia.codigo?.replace(/\D/g, '')) || 1,
          matriculaId: parseInt(paraleloId?.replace(/\D/g, '')) || 1,
          materiaCodigo: materia.codigo,
          materiaNombre: materia.nombre,
          docente: (paralelo?.docente?.nombre || '') + ' ' + (paralelo?.docente?.apellido || ''),
          evaluaciones,
      ...
      console.log('NotasMaterias procesadas:', notasMaterias);
  ...
  async obtenerMisGrupos(): Promise<GrupoDocente[]> {
    ...
    console.log('=== obtenerMisGrupos ===');
    console.log('userId:', userId);
    console.log('usuario:', usuario);
    console.log('docenteCodigo:', docenteCodigo);
    ...
    try {
      ...
      console.log('Paralelos totales:', dtoParalelos);

      // Filtrar solo los paralelos del docente actual
      const misParalelos = dtoParalelos.filter(
        dto => {
          console.log('Comparando docente:', dto.docente?.codigo, '===', docenteCodigo);
          return dto.docente?.codigo === docenteCodigo;
        }
      );
      console.log('Mis paralelos filtrados:', misParalelos);
      ...
      return misParalelos.map(dto => {
        ...
        const estudiantesInscritos = dtoInscripciones.filter(
          insc => insc.paraleloMateria?.codigo === dto.codigo &&
            insc.estado === 'ACEPTADA'
        ).length;

        return {
          grupoId: dto.codigo,  // Usar el código directamente como string
          materiaNombre: dto.materia?.nombre || 'Materia',
          grupoCodigo: dto.codigo,  // El código del paralelo completo
   ...
   */
  async obtenerEstudiantesGrupo(grupoId: string): Promise<EstudianteNotas[]> {
    try {
      console.log('=== obtenerEstudiantesGrupo ===');
      console.log('Buscando estudiantes para grupoId:', grupoId);
      
      // Obtener todas las inscripciones
      const dtoInscripciones = await firstValueFrom(
        this.api.get<any[]>('/inscripciones')
      );
      console.log('Inscripciones totales:', dtoInscripciones);

      // Filtrar inscripciones de este grupo
      const inscripcionesGrupo = dtoInscripciones.filter(
        insc => {
          console.log('Comparando:', insc.paraleloMateria?.codigo, '===', grupoId, 'estado:', insc.estado);
          return insc.paraleloMateria?.codigo === grupoId &&
            insc.estado === 'ACEPTADA';
        }
      );
      console.log('Inscripciones filtradas para el grupo:', inscripcionesGrupo);

      // Obtener todas las calificaciones
      const dtoCalificaciones = await firstValueFrom(
        this.api.get<any[]>('/calificaciones')
      );
      console.log('Calificaciones totales del backend:', dtoCalificaciones);
      ...
      return inscripcionesGrupo.map(inscripcion => {
        ...
        const calificacionesEstudiante = dtoCalificaciones.filter(
          calif => calif.estudiante?.codigo === estudiante.codigo &&
            calif.evaluacion?.paraleloMateria?.codigo === grupoId
        );
        console.log(`Calificaciones de ${estudiante.codigo} en grupo ${grupoId}:`, calificacionesEstudiante);
        ...
        calificacionesEstudiante.forEach(calif => {
          ...
          const nombreEval = calif.evaluacion?.nombre?.toLowerCase() || '';
          // El backend usa 'valor', no 'nota'
          const valorNota = calif.valor ?? calif.nota;

          if (nombreEval.includes('parcial 1') || descripcion.includes('primer') || tipo === 'PARCIAL_1') {
            notas['Parcial 1'] = valorNota;
          } else if (nombreEval.includes('parcial 2') || descripcion.includes('segundo') || tipo === 'PARCIAL_2') {
            notas['Parcial 2'] = valorNota;
          } else if (nombreEval.includes('proyecto') || descripcion.includes('proyecto') || tipo === 'PROYECTO') {
            notas['Proyecto'] = valorNota;
          } else if (nombreEval.includes('final') || descripcion.includes('final') || tipo === 'FINAL') {
            notas['Examen Final'] = valorNota;
          }

          // Calcular nota ponderada
          const porcentaje = calif.evaluacion?.porcentaje || 0;
          if (valorNota !== null && valorNota !== undefined) {
            notaFinal += (valorNota * porcentaje / 100);
            porcentajeEvaluado += porcentaje;
          }
        });

        return {
          estudianteId: estudiante.codigo,
   ...
   * Método fallback con datos mock
   */
  private obtenerEstudiantesGrupoMock(grupoId: string): EstudianteNotas[] {
    const matriculasGrupo = this.datosMock.matriculas.filter(
      m => m.grupo.id.toString() === grupoId && m.estado === 'INSCRITO'
    );
    ...
    return matriculasGrupo.map(matricula => {
      ...
      return {
        estudianteId: (matricula.estudiante as any).codigoEstudiante || 'EST-' + matricula.estudiante.id,
  ...
  private async guardarNotaBackend(
    ...
    try {
      ...
      const inscripcion = dtoInscripciones.find(i =>
        i.estudiante.codigo === estudianteId &&
        i.paraleloMateria.codigo === grupoId
      );

      if (!inscripcion) {
        console.log('Fallback a mock: inscripción no encontrada para', estudianteId, 'en', grupoId);
      ...
      return { exito: false, mensaje: 'Error al guardar nota en el servidor' };
    }
  }
  ...
  private guardarNotaMock(
    ...
    // Este método solo se usa como fallback cuando no hay backend
    // Por ahora, simplemente retornar éxito ya que el backend está activo
    return { exito: true, mensaje: 'Nota guardada (modo demo)' };
  }
